...
boxplot(prospective_data$creat_diff_rate, prospective_data$CKD,
        names = c("With CKD", "Without CKD"),
        ylab = "Creatinine changes rate, µmol/L/month")
```
...
boxplot(prospective_data$mdrd_diff_rate, prospective_data$CKD,
        names = c("With CKD", "Without CKD"),
        ylab = "MDRD eGFR changes rate, mL/min/1.73 m2/month")
```
...
boxplot(prospective_data$cocroft_diff_rate, prospective_data$CKD,
        names = c("With CKD", "Without CKD"),
        ylab = "Cocroft-Gault changes rate, mL/min/month")
```
...
boxplot(prospective_data$ckdepi_diff_rate, prospective_data$CKD,
        names = c("With CKD", "Without CKD"),
        ylab = "CKD-EPI eGFR changes rate, mL/min/1.73 m2/month")
```
...
ggplot(data = melted_tmp, aes(x=clear_drug, y=value)) +
  ...
  labs(x = "", y = "Creatinine, µmol/L ") +
  scale_fill_discrete(labels = c("Starting value", "Final value"))
```
...
ggplot(data = melted_tmp, aes(x = clear_drug, y = value)) +
  geom_boxplot(aes(fill = variable)) +
  labs(x = "", y = "Cokcroft-Gault, mL/min") +
  scale_fill_discrete(labels = c("Starting value", "Final value"))
```
...
ggplot(data = melted_tmp, aes(x = clear_drug, y = value)) +
  geom_boxplot(aes(fill = variable)) +
  labs(x = "", y = "CKD-EPI, mL/min/1.73m2") +
  scale_fill_discrete(labels = c("Starting value", "Final value"))
```
