...
barplot(table(first_visit_data$CKD_Stage1, first_visit_data$clear_drug) %>% prop.table(., 2),
        col = brewer.pal(n = first_visit_data$CKD_Stage1 %>% unique() %>% length, "Blues"),
        main = "CKD stage by drug",
        ylab = "CKD stage proportion"
        )
...
legend(
  ...
  col = brewer.pal(n = first_visit_data$CKD_Stage1 %>% unique() %>% length, "Blues") %>% rev(),
  pch = 15,
  xpd = TRUE,
  inset = c(-0.50, 0)
)
